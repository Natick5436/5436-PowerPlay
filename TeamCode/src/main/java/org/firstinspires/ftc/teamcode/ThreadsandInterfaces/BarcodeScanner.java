...
public class BarcodeScanner extends OpenCvPipeline {
    ...
    static final Scalar HIGH = new Scalar(H + T, 255, 255);
    /*
    ...
    public void init(Mat firstFrame) {
         ...
         */
        region1_HSV = filter.submat(new Rect(region1_pointA, region1_pointB));
        region2_HSV = filter.submat(new Rect(region2_pointA, region2_pointB));
        region3_HSV = filter.submat(new Rect(region3_pointA, region3_pointB));
    }
